...
########## dosen't train sde just use reconstructed signal: max_len 197;
parser.add_argument('--training_len', type=int, default=197)
parser.add_argument('--max_len', type=int, default=197)

parser.add_argument('--isUseSampler', action='store_true', default=False,
...
parser.add_argument('--sgcn_thredgraph', type=float, default=0.1)
parser.add_argument('--pooling', type=str, default="concat", help='concat, sum')
...
if args.keep_files:
    copy('main_sgcn.py', args.res_dir)
    copy('utils.py', args.res_dir)
    copy('kernel/sgcn.py', args.res_dir)
    copy('kernel/train_eval_sgcn.py', args.res_dir)
...
for dataset_name in product(datasets):
    ...
    for num_layers, hidden in combinations:
        ...
        adni_dataset, fmri_subid, target = load_recons_fmri_AllADNI(root='./data', disease_id=args.disease_id, isUseDXLabelOnly=args.isUseDXLabelOnly, isUseDXLabelwithBaselineOnly=args.isUseDXLabelwithBaselineOnly,
                                 isTestConversion=args.isTestConversion)
        # adni_dataset, len_raw_signal, sig_mask, fmri_subid, target = load_fmri(root='./data', disease_id=args.disease_id, isUseDXLabel=args.isUseDXLabel)
